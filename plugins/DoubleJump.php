<?php
/**
 * Allows clients to double-jump, aka. give them velocity in the rough direction they're looking when they jump twice in quick succession similar to initiating flight.
 *
 * @var Plugin $this
 */
use Phpcraft\
...
$this->on(function(ServerFlyingChangeEvent $event)
{
	if($event->client->flying && ($event->client->gamemode == Gamemode::SURVIVAL || $event->client->gamemode == Gamemode::ADVENTURE))
	{
		...
		$y = (1 - $y_perc) * 7 + 1;
		$z = cos(pi() / 180 * $con->yaw) * $y_perc * 13;
